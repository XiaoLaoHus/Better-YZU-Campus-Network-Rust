...
**Windows 图形模式无需手动创建文件**：首次启动直接填写账号、密码并选择服务，点击 **保存并连接**。其他参数使用默认值。已有有效配置会在启动时自动连接。
...
1. 解压 Windows 发行包，双击 `better-yzu-campus-network.exe`。首次启动显示设置表单，即使使用 `--minimized` 也不会隐藏缺少配置的窗口。
2. 填写 **学工号 / 账号**、**校园网密码**，在原生下拉框选择 **网络服务**，点击 **保存并连接**。密码输入会遮蔽；有效的已有配置会自动连接。
...
src/
  ...
  windows_style.rs           GDI 背景与圆角自绘按钮皮肤
