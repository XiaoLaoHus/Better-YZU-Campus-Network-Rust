...
### 1\. 功能概述
  ...
  * **参数自动获取：** 启动时先探测网关，从重定向地址中现取本次连接的会话参数，用户无需手动复制复杂的认证 URL。参数按设备与当次连接生成，因此不会硬编码在程序里。
...
### 5\. 故障排除
  ...
  * **`当前已在线，跳过本次登录。`：** 网关把门户请求重定向到了认证成功页，说明本机已在线上，无需重复登录。想测试登录流程，需要先通过自助服务页面退出登录。
  * **`未能从网关取到认证入口，跳过本次登录。`：** 网关有响应但没有给出认证入口，说明当前的探测方式对这个网络不适用。每次探测都会打印 `探测 <地址> ...` 及其 HTTP 状态和跳转目标，**跳转地址中的参数值不会写进日志**，可据此判断链路走到了哪一步。
...
### 6\. 从 Python 版迁移的差异
  ...
  * 认证 URL 从源码硬编码改为运行时探测：Python 版内置了某一台设备的完整 SSO 入口 URL，其中的 IP、MAC 等参数只对那台设备有效；Rust 版改为每次登录前探测网关、从重定向地址现取参数。
