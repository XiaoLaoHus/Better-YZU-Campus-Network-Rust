...
### 1\. 功能概述
  ...
  * **参数自动获取：** 启动时先探测网关，现取本次连接的会话参数，用户无需手动复制复杂的认证 URL。参数按设备与当次连接生成，因此不会硬编码在程序里。入口取两种形态：网关的重定向地址（SSO 登录页），以及连接校园网时系统弹出的那个认证网页的地址（会话参数直接写在地址里）。
...
### 5\. 故障排除
  ...
  * **`未能从网关取到认证入口，跳过本次登录。`：** 网关有响应但没有给出认证入口，说明当前的探测方式对这个网络不适用。每次探测都会打印 `探测 <地址> ...` 及其 HTTP 状态和跳转目标；网关若不跳转而是直接返回网页，还会打印正文里找到的链接。**地址中的参数值不会写进日志**，可据此判断链路走到了哪一步。
